...
public class PdsController {
	...
	public String modifyPOST(PdsModifyCommand modofyReq, HttpServletRequest request, RedirectAttributes rttr)throws Exception {
		String url = "redirect:/pds/detail.do";
		
		// 파일 삭제
		if(modofyReq.getDeleteFile() != null && modofyReq.getDeleteFile().length > 0) {
			for(String anoStr : modofyReq.getDeleteFile()) {
		...
		// 파일저장
		List<AttachVO> attachList = GetAttachesByMultipartFileAdapter.save(modofyReq.getUploadFile(), fileUploadPath);
		
		// PdsVO setting
		PdsVO pds = modofyReq.toPdsVO();
		pds.setAttachList(attachList);
		pds.setTitle((String)request.getAttribute("XSStitle"));
